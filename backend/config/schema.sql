...
-- indexes for better query performance O(logn) instead of O(n) scanning the whole table
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE t.tgname = 'update_users_timestamp'
          AND c.relname = 'users'
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname = 'update_user_preferences_timestamp'
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname = 'update_pantry_items_timestamp'
          AND c.relname = 'pantry_items'
          AND n.nspname = current_schema()
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname = 'update_recipes_timestamp'
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname = 'update_meal_plans_timestamp'
          AND c.relname = 'meal_plans'
          AND n.nspname = current_schema()
    ) THEN
        CREATE TRIGGER update_meal_plans_timestamp BEFORE UPDATE ON meal_plans
            FOR EACH ROW EXECUTE FUNCTION update_timestamp();
    END IF;
END$$;
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN pg_class c ON t.tgrelid = c.oid
        JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE t.tgname = 'update_shopping_lists_timestamp'
          AND c.relname = 'shopping_lists'
          AND n.nspname = current_schema()
    ) THEN
        CREATE TRIGGER update_shopping_lists_timestamp BEFORE UPDATE ON shopping_lists
            FOR EACH ROW EXECUTE FUNCTION update_timestamp();
    END IF;
END$$;
