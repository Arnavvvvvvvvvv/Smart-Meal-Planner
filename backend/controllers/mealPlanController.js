...
export const addToMealPlan = async (req, res, next) => {
    try {
        ...
        res.status(201).json({success: true,message: 'Recipe added to meal plan',data: { mealPlan }});
    } catch (error) {
        next(error);
    }
};
...
export const getWeeklyPlan = async (req, res, next) => {
    try {
        ...
        if (!startDate) {
            return res.status(400).json({success: false,message: 'Please provide start_date or weekStartDate'});
        }
...
export const deleteMealPlan = async (req, res, next) => {
    try {
        ...
        res.json({success: true,message: 'Meal plan entry deleted successfully'});
