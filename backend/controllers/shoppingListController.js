...
export const generateFromMealPlan= async(req,res,next)=>{
    try{
        ...
        if (!startDate || !endDate) {
            return res.status(400).json({success: false,message: 'Please provide startdate & enddate'});
        }
        const items= await ShoppingList.generateFromMealPlan(req.user.id, startDate, endDate);
        res.json({success:true, message:'Shopping list generated from meal plan', data:{items}})
    }catch(err){
        next(err);
    }
}
...
export const getShoppingList= async (req,res,next)=>{
    try{
        ...
        const items = grouped? await ShoppingList.groupByCategory(req.user.id): await ShoppingList.findByUserId(req.user.id);
...
export const addItem = async (req, res, next) => {
    try {
        ...
        res.status(201).json({success: true,message: 'Item added to shopping list',data: { item }});
    } catch (error) {
        next(error);
    }
};
...
export const updateItem= async(req,res,next)=>{
    try {
        ...
        res.status(201).json({success: true,message: 'Item updated',data: { item }});
    } catch (error) {
        next(error);
    }
}
...
export const clearChecked = async (req, res, next) => {
    try {
        ...
        res.json({success: true,message: 'Checked items cleared',data: { items }});
    } catch (error) {
        next(error);
    }
};
...
export const clearAll = async (req, res, next) => {
    try {
        ...
        res.json({success: true,message: 'Shopping list cleared',data: { items }});
    } catch (error) {
        next(error);
    }
};
...
export const addCheckedToPantry = async (req, res, next) => {
    try {
        ...
        res.json({success: true,message: 'Checked items added to pantry',data: { items }});
    } catch (error) {
        next(error);
    }
};